...
export default function OnboardingWizard() {
  ...
  const ctlDebounceRef = useRef<NodeJS.Timeout | null>(null);
  ...
  const panResponder = useRef(
    PanResponder.create({
      onMoveShouldSetPanResponderCapture: (_, gestureState) => {
        return gestureState.dx > 12 && Math.abs(gestureState.dx) > Math.abs(gestureState.dy) * 1.2;
      },
          ...
          >
            ...
            <View className="flex-row items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-zinc-800">
              ...
              <TouchableOpacity onPress={handleConfirmWheelDate}>
                <Text className="text-emerald-600 dark:text-emerald-400 text-base font-bold">Confirm</Text>
              </TouchableOpacity>
            </View>
      ...
      <View {...panResponder.panHandlers} className="flex-1">
        ...
        {currentStep === 1 && (
          <View className="space-y-4">
            ...
            <Text className="text-theme-text text-sm">
              Select how Coach Nana communicates with you during post-workout feedback and chat.
            </Text>
            ...
            >
              <View className="flex-row items-center">
                <View className="w-12 h-12 rounded-full bg-theme-accent/20 items-center justify-center mr-3">
                  <Ionicons name="sparkles" size={24} color="#208AEF" />
                </View>
            ...
            >
              <View className="flex-row items-center">
                ...
                <View className="flex-1">
                  <Text className="text-theme-text font-bold text-base">Cheerleader</Text>
                  <Text className="text-theme-muted text-xs mt-1">
                    Super enthusiastic, positive, forgiving on missed sessions.
                  </Text>
                </View>
              </View>
            </TouchableOpacity>
        ...
        {currentStep === 2 && (
          <View className="space-y-4">
            ...
            <Text className="text-theme-text text-sm">
              Provide context to help Nana tailor your workouts around work, family, and preferences.
            </Text>
          ...
          <View className="space-y-4">
            ...
            <View className="bg-theme-card border border-theme-border rounded-2xl p-5">
              ...
              <Text className="text-theme-muted text-xs leading-relaxed">
                Spark is your core XP currency. Every run, cycle, swim, or strength session logged earns Spark points based on intensity, duration, and TSS load.
              </Text>
            </View>

            <View className="bg-theme-card border border-theme-border rounded-2xl p-5">
              <View className="w-12 h-12 rounded-full bg-theme-accent/20 items-center justify-center mb-3">
                <Ionicons name="trending-up" size={26} color="#208AEF" />
              </View>
              <Text className="text-theme-text font-bold text-lg mb-1">Leveling Up</Text>
              <Text className="text-theme-muted text-xs leading-relaxed">
                As your cumulative Spark grows, you unlock higher Spark Levels. Your rank is showcased on community leaderboards, and Coach Nana celebrates your level milestones!
              </Text>
            </View>
          </View>
        )}

        {/* STEP 4: SCHEDULE BOUNDARIES */}
        {currentStep === 4 && (
          <View className="space-y-4">
            <Text className="text-theme-muted text-xs font-bold uppercase tracking-wider">
            ...
            <Text className="text-theme-text text-sm">
              Configure available days and maximum training minutes. Nana will distribute weekly volume within these bounds.
            </Text>

            {DAYS.map((day) => {
              const dayData = availability[day] || { available: false, maxMinutes: 60 };
              return (
                <View
                  key={day}
                  className="flex-row items-center justify-between p-3 bg-theme-card border border-theme-border rounded-xl"
                >
                  ...
                  {dayData.available ? (
                    <View className="flex-row items-center space-x-2">
                      <TextInput
                        keyboardType="numeric"
                        value={dayData.maxMinutes.toString()}
                        onChangeText={(val) =>
                          setAvailability({
                            ...availability,
                            [day]: { ...dayData, maxMinutes: parseInt(val, 10) || 0 },
                          })
                        }
                        className="w-16 p-2 bg-theme-bg border border-theme-border rounded-lg text-theme-text text-center text-sm font-bold"
                      />
                      <Text className="text-theme-muted text-xs font-bold">mins</Text>
                    </View>
                  ) : (
                    <Text className="text-theme-muted text-xs italic">Rest Day</Text>
                  )}
                </View>
              );
            })}
        ...
        {/* STEP 5: INTEGRATIONS & GOALS */}
        {currentStep === 5 && (
          <View className="space-y-4">
            <Text className="text-theme-muted text-xs font-bold uppercase tracking-wider">
              5. Integrations & Race Target
            </Text>

            {/* Garmin Connect */}
            <View className="bg-theme-card border border-theme-border rounded-xl p-4">
              <TouchableOpacity
                onPress={() => setShowGarmin(!showGarmin)}
                className="flex-row items-center justify-between"
              >
                <View className="flex-row items-center">
                  <Ionicons name="watch" size={20} color="#208AEF" className="mr-3" />
                  <Text className="text-theme-text font-bold text-sm">Garmin Connect</Text>
                </View>
                <Ionicons name={showGarmin ? 'chevron-up' : 'chevron-down'} size={18} color="#8E8E93" />
              </TouchableOpacity>

              {showGarmin && (
                <View className="mt-3 pt-3 border-t border-theme-border space-y-2">
                  <TextInput
        ...
        {/* STEP 6: SUBSCRIPTION / SPARK PLUS PAYWALL (PLACEHOLDER) */}
        {currentStep === 6 && (
          <View className="space-y-4">
            <View className="items-center my-2">
              ...
              <Text className="text-theme-muted text-xs text-center mt-1 px-4">
                Upgrade to unlock unlimited AI Coach Nana chat tokens, custom macro periodization, automated Garmin sync, and injury diagnostics.
              </Text>
            </View>
            ...
            <View className="bg-theme-card border border-theme-border rounded-2xl p-4 space-y-3">
              {[
                'Increased AI Coach Nana chat tokens',
